...
El an\'alisis de la tasa de error de bits se realizo para ambos modelos de DQPSK proporcionados por
\emph{GNURadio}. El programa que se utilizo fue basado en un estudio generado y publicado en la
pagina de \gnuradio sobre una versi\'on mas optima del modulador GMSK. El programa fue modificado
para que pudiera analizar y comparar ambos esquemas DBPSK y DQPSK en sus dos versiones: costas/MM y
filtros polifasicos.
...
\begin{lstlisting}[float, label=ex:pynoisegen, caption={Codigo que implementa el grafo generador de
ruido}, breaklines=true]
...
%FIXME: Expliar el proposito del filtro pre-detector.
La tercera etapa realiza la demodulaci\'on de las se\~nales capturadas por los grafos anteriores.
Esta etapa consiste en leer la se\~nal mezclada con ruido, aplicar un filtro pre-detector, demodular la
se\~nal recibida y guardar los datos generados en un archivo para realizar la comparaci\'on. La
estructura del grafo que realiza esta etapa se muestra en la figura \ref{fig:analizer}.
...
\begin{figure}[htp]
  ...
  \begin{tikzpicture}[scale=0.8, transform shape]
  	...
  	\node (fir) [grblock, right=of sigsource] {\footnotesize{Filtro pre-detector}}
  	edge [<-] (sigsource);
...
por segundo y los par\'ametros de ancho de banda para el filtro pre-detector. Para este an\'alisis
se utilizaron los valores por default.
