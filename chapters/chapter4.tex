...
conector SMA a BNC para conectar una antena. La impedancia de los conectores es de 50$\Omega$. Los conectores SMA manejan un rango de
frecuencias entre 0 y 18Ghz. La se–al viaja a entre del conductor central y una capa protectora, com\'unmente aluminio. Este
arreglo ofrece una buena protecci\'on contra el ruido externo \cite{coax}.
...
Algunos par\'ametros contienen valores por defecto que se puede utilizar como punto de partida para ajustarlos para la
...
poder optimizarlos. Los valores que se utilicen dependen del \emph{hardware} (USRP, tarjetas, capacidad de la PC) que se este
utilizando para desarrollar la aplicaci\'on. A menos de que se especifique lo contrario, en la mayor\'ia de los par\'ametros de
los diferentes bloques y aplicaciones que se utilizaron para el experimento se utilizaron los valores por defecto ya establecidos
y recomendados debido a que dieron buenos resultados y buen rendimiento. Los resultados se muestran a trav\'es de las secciones de
este cap\'itulo.
...
analisis del BER.}, breaklines=true]
    ...
    fig = p.figure()
    p.title(u'DesempeÃ±o de DQPSK y DBPSK')
...
%TODO: Explicar la funcion get_traces.
