...
\begin{figure}[ht]
\centering
	\includegraphics[width=5in]{figs/usrp}
...
El controlador FX2 utiliza 3 endpoints para la comunicaci\'on con el FPGA y la
PC como se describen en la tabla \ref{tbl:endpoints}. El endpoint 0 es el de
control y es necesaria su implementaci\'on para que el dispositivo pueda ser
compatible y a su vez, ser certificado por el est\'andar de USB \cite{usb}. Los
endpoints 2 y 6 son de tipo Bulk, que de acuerdo a la especificaci\'on del USB,
son los que permiten el mayor rendimiento de transferencia (512 bytes por
paquete). Estos son utilizados para enviar los datos de la se\~nal al FPGA. El
FX2 utiliza una interfaz de prop\'osito gen\'erico (GPIF) para proporcionar un
bus de datos al mundo exterior, esto con el prop\'osito de facilitar la interfaz
a otros dispositivos. En este caso el GPIF se conecta directamente al FPGA con
una tasa de transferencia de 96 MB/sec.
...
DDC (\emph{Digital Down Converters}) implementados con filtros CIC (Peine
Integradores en Cascada) de 4 etapas y filtros de banda media (\emph{Half-band}
o HB) de orden 30. El prop\'osito del DDC es adaptar la frecuencia de la se\~nal
a otra frecuencia que el puerto USB pueda manejar adecuadamente. La estructura
del DDC se muestra en la figura \ref{fig:ddcblock}.
...
EL USRP, en su configuraci\'on b\'asica, implementa 2 DDCs, cada uno con 2
entradas. La se\~nal compleja es multiplicada por otra se\~nal con una
frecuencia intermedia constante.  La se\~nal resultante se encuentra ahora
centrada en 0. Despu\'es es introducida al filtro CIC para realizar una
decimaci\'on por $N$, donde $N$ es especificado por el usuario desde el programa
...
integradora}\\
H_C^N&=\text{es la funci\'on de transferencia de la etapa peine}\\
R&=\text{es la taza de decimaci\'on o interpolaci\'on}\\
M&=\text{numero de muestras por etapa}\\
N&=\text{numero de etapas por filtro}
\end{aligned}
\end{equation*}

Como se mencion\'o anteriormente, el USRP implementa este filtro con los
siguientes par\'ametros: $R$ = variable (4 m\'inimo), $M$ = 1 y $N$ = 4 La
respuesta a la frecuencia de esta configuraci\'on se muestra en la
\ref{fig:cicresp}.
