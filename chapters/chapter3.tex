...
informaci\'on que fluye atreves del grafo y est\'os pueden ser operaciones
...
tipos de datos incompatibles. Un ejemplo de una gr\'afica de flujo sencilla se
...
\begin{figure}[hpt]
  ...
  \vspace{0.3in}
	...
	\end{tikzpicture}
	\vspace{0.5in}
	\caption{Ejemplo de una gr\'afica de flujo en \emph{GNURadio}}
	\label{fig:radioflow}
\end{figure}
...
un contenedor para la gr\'afica de flujo. La primera clase act\'ua como un
contenedor para una \'unica gr\'afica y la segunda soporta el desarrollo de
gr\'aficas jer\'arquicas.
...
\centering
	...
	\end{tikzpicture}
\vspace{0.5in}
\caption{Gr\'afica de flujo del programa ejemplo.}
...
generadas. La gr\'afica representativa de este programa se muestra en la figura
...
usuario utiliando la libreria QT que ayuda a visualizar de manera gr\'afica lo que el sistema esta
...
% El codigo fuente de \gnuradio actualmente contiene los siguientes bloques que implementan estos conceptos:
% \begin{itemize}
%   \item \textbf{gr\_pfb\_arb\_resampler}: Este bloque implementa un re-muestreador arbitrario utilizando un banco de filtros
%   polifasicos. La tasa de re-muestreo puede ser cualquier numero real $r$ y realiza la operacion construyendo $N$ cantidad de
%   filtros donde $N$ es el factor de interpolacion. El factor $D$ se calcula de la siguiente manera: $D=N/r$. Utilizando $N$ y
%   $D$ el bloque realiza un muestreo arbitrario con $N/D$ representando un numero racional cercano a la tasa especificada $r$
%   y utilizando $N$ cantidad de filtros ciclandose atravez de un banco de filtros polifasicos.
%   \item \textbf{gr\_pfb\_clock\_sync}: Este bloque realiza sincronizacion de tiempo para se\~nales PAM. Lo logra minimizando
%   la derivada de la se\~nal filtrada, lo cual maximiza el SNR y minimiza el efecto ISI. Para lograr esto el bloque construye
%   dos bancos de filtros; el primero contiene el filtro de forma de pulso (ejemplo: filtro de coseno elevado). El segundo
%   banco de filtros contiene la derivada de los filtros del primero banco. Si se observa esto desde el dominio del tiempo, el
%   primer banco de filtros contiene filtros que tienen una respuesta al impulso de la forma sinc. El objetivo es alinear la
%   se\~nal de salida de tal forma que sea muestreada justamente en el pico de la funcion sinc. La derivada de la funcion sinc
%   es cero en su punto maximo, o sea, $sinc(0)=1$, $sinc(0)'=0$. Este echo se utiliza para generar una se\~nal de error
%   $e(n)$. Esta se\~nal entrega un valor proporcional a la distancia con respecto al punto cero. El bloque utiliza un PLL de
%   segundo orden para lograr que el error llege a cero.
% \end{itemize}